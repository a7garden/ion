...
export function MyPage({ onLogout }: MyPageProps) {
  const { state, removePost, changeDisplayName } = useApp();
  ...
  const handleSaveName = async () => {
    ...
    await changeDisplayName(newDisplayName.trim());
  ...
  const handleDelete = async (postId: string) => {
    ...
    toast({ description: ok ? '삭제되었습니다' : '삭제에 실패했습니다', duration: 2000 });
        ...
        <div className="relative py-6 sm:py-8 text-center">
          ...
          >
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-accent to-accent/60 flex items-center justify-center shadow-lg warm-glow">
              {state.userAvatar ? (
                <img src={state.userAvatar} alt="" className="w-full h-full rounded-full object-cover" />
              ) : (
            ...
            <div>
              {isEditingName ? (
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => { setNewDisplayName(state.userName || ''); setIsEditingName(false); }}><X className="w-4 h-4" /></Button>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">{state.userName}</h1>
                    ...
                    {post.media && (
                      <div className="mb-3 sm:mb-4 rounded-xl overflow-hidden">
                        {post.mediaType === 'video' ? (
                          <video src={post.media} className="w-full max-h-[250px] object-cover" controls />
                        ) : (
                          <img src={post.media} alt="" className="w-full max-h-[250px] object-cover" />
                        )}
                      </div>
                    )}
                    {post.content && (
