    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[360px] w-[calc(100vw-2rem)] rounded-2xl sm:rounded-3xl p-0 gap-0 overflow-hidden border-border/50 warm-glow">
        ...
        <DialogHeader className="relative px-5 sm:px-6 pt-5 sm:pt-6 pb-3 sm:pb-4 text-center">
          ...
          <DialogTitle className="text-lg sm:text-xl font-semibold text-foreground">{t('login.welcome')}</DialogTitle>
          ...
          <div className="space-y-2">
            <motion.div whileTap={{ scale: 0.98 }}>
              <Button className="w-full bg-accent hover:bg-accent/90 text-accent-foreground font-medium rounded-xl touch-target"
                ...
                <LogIn className="w-4 h-4 mr-2" />{t('login.google')}
              </Button>
            </motion.div>
            <Button variant="ghost" className="w-full rounded-xl touch-target" onClick={() => onOpenChange(false)}>{t('login.cancel')}</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
